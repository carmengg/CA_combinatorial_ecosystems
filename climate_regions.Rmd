...
# return raster brick with avg temperatures for every month over [year-n,year-1]
# ex: year=2001, n=30, then average over [1971,2000]
# bbox = bounding box of area of interest
...
# temp_regions = raster with temperature regions coming from pre_temp_regions (codes 1,3-6 and 1= polar or boreal)
...
separate_polar_boreal <- function(temp_regions, months_weather){
  ...
  n <- length(months_boreal)
  thresh <- matrix(c(-Inf, 10, 0, 10, Inf, 1), nrow=2,byrow=TRUE)
  for(i in 1:n){
    months_boreal[[i]] <- reclassify(months_boreal[[i]],thresh) 
  }
  boreal_region <- sum(months_boreal)  
  boreal_region[boreal_region>0] <- 1 # here every boreal pixel = 1, polar pixels = 0, all others NA
